    ...
    for name, method in cls_inst.__dict__.items():
        if (callable(method) and _is_parametric( {"instructions":[{"name": name}]} )):
            ...
            def remembering_wrapper(parametric_method):
                ...
                def wrapper(*args, param_name: str = "NoName", **kwargs):

                    cls_inst.param_instructions.append(param_name) # save the name of the parameter
            ...
            if _is_parametric(instruction):
                if "param_name" in instruction:
                    ...
                    if instruction["name"] in three_params: # three params: theta, phi, lam
                        cls_inst.param_instructions + [instruction["param_name"], instruction["param_name"], instruction["param_name"]]
                        if not "theta" in instruction:
                            instruction["theta"] = 0.
                        if not "phi" in instruction:
                            instruction["phi"] = 0.
                    ...
                    if instruction["name"] in four_params: # four params: theta, phi, lam, gamma
                        cls_inst.param_instructions + [instruction["param_name"], instruction["param_name"], instruction["param_name"], instruction["param_name"]]
                        if not "theta" in instruction:
                            instruction["theta"] = 0.
                        if not "phi" in instruction:
                            instruction["phi"] = 0.
    ...
    def total_order_params(self, recompile = False, **params: dict[str, Union[int, float, list[Union[int, float]]]]) -> list[Union[int, float]]:
        ...
        Args:
            ...
            **params (dict): any parameters to be input, using keyword arguments with the format
                             param_name1 = value1, param_name2 = [value2_a, value2_b]
...
def EMZC2(nE, nM, repeat_encode, repeat_evolution):
    ...
    for qubit in range(nE):
            emcz2.encoder.ry(qubit, param_name = "x")
            emcz2.encoder.rx(qubit, param_name = "theta")
            emcz2.encoder.rz(qubit, param_name = "theta")

    for qubit in range(nE):
        emcz2.final_encoding.ry(qubit, param_name = "x")

    # Define evolution
    for qubit in range(nE+nM-1): # Last qubit missing

        emcz2.evolver.rx(qubit, param_name = "theta")
        emcz2.evolver.rz(qubit, param_name = "theta")
        emcz2.evolver.cz(qubit, qubit + 1)
         
    emcz2.evolver.rx(nE + nM - 1, param_name = "theta") # I separate the last iteration beacause it doesn't have a CZ with the next qubit
    emcz2.evolver.rz(nE + nM - 1, param_name = "theta")

    for qubit in range(nE):
        emcz2.final_evolution.rx(qubit, param_name = "theta")
...
def EMZC3(nE, nM, repeat_encode, repeat_evolution):
    ...
    # Define encoding
    for qubit in range(nE):
            emcz3.encoder.ry(qubit, param_name = "x")
            emcz3.encoder.rx(qubit, param_name = "theta")
            emcz3.encoder.rz(qubit, param_name = "theta")
            emcz3.encoder.rx(qubit, param_name = "theta")

    for qubit in range(nE):
        emcz3.final_encoding.ry(qubit, param_name = "x")

    # Define evolution
    # E register part (CZ are performed)
    for qubit in range(nE): 

        emcz3.evolver.rx(qubit, param_name = "theta")
        emcz3.evolver.rz(qubit, param_name = "theta")
        emcz3.evolver.rx(qubit, param_name = "theta")
    ...
    for qubit in range(nE):
        emcz3.final_evolution.rx(qubit, param_name = "theta")
        emcz3.final_evolution.rz(qubit, param_name = "theta")
        emcz3.final_evolution.rx(qubit, param_name = "theta")
